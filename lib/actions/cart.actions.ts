...
export async function addItemToCart(data: CartItem) {
  try {
    ...
    if (!cart) {
      ...
      const existItem = (cart.items as CartItem[]).find((exsit) => exsit.productId === item.productId)
      if (existItem) {
        ...
        (cart.items as CartItem[]).find((exsit) => exsit.productId === item.productId)!.qty = existItem.qty + 1
